...
for h in range(1, 25):
    ...
    p_hour = df[df['HH'] == h]['Zonal load'].values
    daily_price = df['Zonal load'].values
    ...
    # 1st Naive Forecast
    pf_naive1 = df['Zonal load'].shift(7 * 24)
    ...
    pf_naive2 = df['Zonal load'].shift(1 * 24)
    included_days = ['Monday', 'Saturday', 'Sunday']
    df['Naive forecast 2'] = df['Zonal load']
    df.loc[df['day_of_week'].isin(included_days), 'Naive forecast 2'] = df['Zonal load'].shift(7 * 24)
    ...
    # 3rd Naive Forecast
    pf_naive3 = df['Zonal load'].shift(1 * 24)
...
daily_avg_price = df.groupby('YYYYMMDD')['Zonal load'].mean()
...
# 2nd Naive Forecast
pf_naive2_daily = daily_avg_combined['Zonal load'].shift(1) #domyślnie
